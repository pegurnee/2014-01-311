...
public class SortedList {
	...
	public class SortedIterator {
		...
		public void delete() {
			if (this.position == null) {
				...
				// Deleting last node
				this.position.prev.next = null; // destroys the link from the
												// last node
				this.position = null;
			} else {
	...
	public boolean delete(int theRecordNum) {
		SortedIterator i = new SortedIterator();
		while (i.position != null) {
			if (i.position.data.getRecordNumber() == theRecordNum) {
				break;
			}
	...
	public int find(String target) {
		...
		while (i.hasNext()) {
			count++;
			if (i.next().getKey().compareTo(target) == 0) {
				break;
			}
		}
		
		return count == 0 ? this.nElems : count;
	}
